...
FUNCTION gui_prompt, promptText, TITLE=windowTitle, VALIDATE_TYPE=datatype, XSIZE=nCharacters, XOFFSET=xoffset, YOFFSET=yoffset, _EXTRA=_extra
	...
	if n_elements(windowTitle) eq 0 then winTitle = 'Entry Form'
		...
		wPromptBox = cw_field(tlb, TITLE=promptText, FLOATING=validateReal, LONG=validateLong, UVALUE=pReturnValue, /RETURN_EVENTS, XSIZE=nCharacters, _EXTRA=_extra)
		widget_control, tlb, /REALIZE
		xmanager, 'gui_prompt', tlb
		if n_elements(*pReturnValue) eq 0 then returnValue='' else returnValue = strtrim(*pReturnValue, 2)
